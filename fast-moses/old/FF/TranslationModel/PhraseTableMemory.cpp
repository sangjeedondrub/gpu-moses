...
PhraseTableMemory::PhraseTableMemory(const std::string &line, const System &system)
...
{
  InputFileStream iniStrme(m_path);
  ...
  while (getline(iniStrme, line)) {
    ...
    TargetPhrase *target = TargetPhrase::CreateFromString(*this, toks[1], toks[2], true, m_system);
